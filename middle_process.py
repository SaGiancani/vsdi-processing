        ...
        for row, subfig in enumerate(subfigs):
            ...
            for i, ax in enumerate(axs):
                ...
                if i < len(mask):
                    ...
                    print(len(sig))
                    print(cdi_select)
                    ax.errorbar(np.arange(0, self.header['n_frames']-1, 1), np.mean(sig[cdi_select, :], axis = 0), yerr=(np.std(sig[cdi_select, :], axis = 0)/np.sqrt(len(cdi_select))), fmt='--', color = 'k', elinewidth = 0.5)
